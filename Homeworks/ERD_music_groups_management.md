...
groups					
	...
	chart_position	INT	Позиция в последнем хит-параде	> 0	"Вычисляемый атрибут - наименьшая позиция песен группы в последнем чарте.

Последний чарт определяется по максимальной дате (chart_date) в таблице charts."
group_members					
...
tours					
	...
	start_date	DATE	Дата начала выступлений		"Вычисляемый атрибут - дата первого концерта, входящего в тур

average_ticket_price =
AVG(concerts.ticket_price)
WHERE concerts.tour_id = tours.tour_id

Использовать триггер - при добавлении или изменении концерта 
   - пересчитать average_ticket_price
   - обновить start_date и end_date"
	end_date	DATE	Дата окончания выступлений		"Вычисляемый атрибут - дата последнего концерта, входящего в тур

start_date =
MIN(concerts.concert_date)
WHERE concerts.tour_id = tours.tour_id

Использовать триггер - при добавлении или изменении концерта 
   - пересчитать average_ticket_price
   - обновить start_date и end_date"
	average_ticket_price	DECIMAL(10,2)	Средняя цена билета		"Вычисляемый атрибут - 
среднее значение ticket_price по всем концертам, входящим в тур

end_date =
MAX(concerts.concert_date)
WHERE concerts.tour_id = tours.tour_id
